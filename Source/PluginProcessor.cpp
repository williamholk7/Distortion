...
#endif
{
//    addParameter(bits = new AudioParameterFloat("bitParam", // string for identifying parameter in code
//                                    "Bits", // string shown in DAW to user
...
AudioProcessorValueTreeState::ParameterLayout DistortionAudioProcessor::createParameterLayout(){
    ...
    params.push_back(std::make_unique<AudioParameterFloat>("bitParameter", "Bits", 1.f, 16.f, 4.f));
...
void DistortionAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float bitNumber = *state.getRawParameterValue("bitParameter");
    distortion.setBits(bitNumber);
